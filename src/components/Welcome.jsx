import gsap from 'gsap'

import { useGSAP } from '@gsap/react'
import React, { useRef } from 'react'

const FONT_W={
    subtitle :{min:100,max:400,default:100},
    title: {min:400,max:900,default:400}
}

const renderText= (text,className,basew=400)=>{
    return [... text].map((char,i)=>(
        <span key={i} className={className}
        style={{fontVariationSettings:`'wght' ${basew}`}}>
            {char=== " "?'\u00A0' : char}
        </span>
    ))
}


const setupTextHover =(container,type)=>{
    if(!container) return;

    const letters =container.querySelectorAll("span");
    const {min,max,default:base} =FONT_W[type];

    const animatelet = (letter,weig,duration=.25)=>{
    return gsap.to(letter,{duration,ease: "power2.out",
        fontVariationSettings: `'wght' ${weig}`
    });

    
}

    const hanldeMouseMove = (e)=>{
        const {left} =container.getBoundingClientRect();
        const mouseX=e.clientX-left;

        letters.forEach((letter)=>{
            const {left:l,width:w} =letter.getBoundingClientRect();
            const dist=Math.abs(mouseX-(l-left +w/2));
            const intensity= Math.exp(-(dist**2)/2000);
            animatelet(letter,min+(max-min)*intensity);
        })
    }

    const hanldeMouseLeave =()=>{
        letters.forEach((letter)=> animatelet(letter,base,0.3))
    }

    container.addEventListener("mousemove",hanldeMouseMove);
        container.addEventListener("mouseleave",hanldeMouseLeave);

        return ()=>{
            container.addEventListener("mousemove",hanldeMouseMove);
        container.addEventListener("mouseleave",hanldeMouseLeave);

        }
}

function Welcome() {

        const titleRef=useRef(null);
        const subtitleRef=useRef(null);
        
        useGSAP(()=>{
            const titleclean=setupTextHover(titleRef.current,"title");
            const subtitleclean=setupTextHover(subtitleRef.current,"subtitle");
            
            return ()=>{
                titleclean();
                subtitleclean();
            }
        },[]);
        return <section id="welcome" >
            <p ref={subtitleRef}>
                {renderText("You're in my portfolio now.",
                    'text-3xl font-georama',100
                )}</p>
            <h1 ref={titleRef} className='mt-7'>{
            renderText(" I'm Adi.",'text-9xl italic font-georama')}

            </h1>

            <div className='small-screen'>
                <p>This portfolio is designed for desktop/tablet screen only.</p>
            </div>
        </section>
    
}

export default Welcome
 